...
async def fetch_figma_frames_via_api(
    ...
    *,
    ...
    """逐帧渲染设计稿 —— 经宿主助手发 Figma REST API。

    容器直连 api.figma.com 会被数据中心 IP 的 WAF 403(与浏览器同源问题),
    故由跑在宿主的 figma_runner(/frames)用住宅 IP 代发。对 view-only 文件同样有效。
    宿主返回 [{name, png_b64}] → 这里逐帧落盘。返回 {ok, path, frames, error}。
    """
    import base64
    import httpx
    if not token:
    ...
    try:
        async with httpx.AsyncClient(timeout=180.0) as cli:
            r = await cli.post(f"{HOST_RUNNER_URL}/frames", json={
                "file_key": file_key, "token": token,
                "prefer_node": prefer_node or "", "max_frames": max_frames})
        if r.status_code != 200:
            return {"ok": False, "error": f"宿主助手 /frames HTTP {r.status_code}"}
        data = r.json()
        if not data.get("ok"):
