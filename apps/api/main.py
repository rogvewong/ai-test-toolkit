...
TOOL_CATALOG: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "input": {
            ...
            "hint": (
                "⚠️ 本工具是「真机证据分析」型 —— 不在容器内自动截图。容器里的桌面 Chromium 模拟不出 "
                "iOS Safari / 微信 X5 / 各 WebView 的真实渲染,低保真截图没有诊断价值,故已停用。\n"
                "**必须先在宿主机用 scripts/h5_device_collect.py 采三端(iOS/Android/Web)真机证据,"
                "生成 evidence.md 粘进来**,工具才能做适配深析;只给 URL 不给证据 → 工具会如实退回要求补采。\n\n"
                "建议格式：\n"
                "## 待测 H5 URL\n<https://...>\n\n"
                "## 真机证据(evidence.md 内容)\n<scripts/h5_device_collect.py 采集的三端 DOM 真值 / 截图 / UA / scrollWidth 等>\n\n"
                "## 业务/客诉/已知差异\n<页面用途 / 已知兼容问题>"
            ),
            "primary_key": "documents",
            "format": "text",
...
async def _seo_synthesize(ctx: Any, state: dict[str, Any]) -> dict[str, Any]:
    ...
    system = (
        ...
        "【站点类型判定 · 最高优先级 · 防臆测,违者结论作废】站点类型必须**仅从采集数据判定**"
        "(看 templates 模板分类、pages_with_video_jsonld、实际页面内容/标签),**严禁预设任何品类**。"
        "**仅当采集数据确实显示存在视频内容(pages_with_video_jsonld>0,或模板含视频详情页/页面有 <video>/VideoObject)时**,"
        "才启用下文 ④⑥ 及检查清单里所有涉及『视频 / VideoObject / 视频 sitemap』的检查项;"
        "**若采集数据没有视频内容,这些视频维度一律标『不适用 N/A』,绝不把『缺 VideoObject』当缺陷、"
        "绝不臆测该站为视频站、绝不输出任何视频相关 issue / 整改项 / 富结果建议**。"
        "对非视频站(博客 / 电商 / 企业站 / 工具站 / 落地页 / 文档站等),按其**实际类型**套用对应 SEO 标准。\n\n"
        ...
        "④ 结构化数据(富结果):JSON-LD 类型应与**实际页面类型**匹配(文章→Article、商品→Product、FAQ→FAQPage 等)。"
        "**仅当确为视频站(见上『站点类型判定』护栏)**:VideoObject 覆盖 pages_with_video_jsonld 对比视频详情模板页数,视频页系统性无 VideoObject 才是 high+ 缺陷;**非视频站不得提 VideoObject**;"
        ...
        "**若 cwv 来自容器单次合成抓取(非真实用户/CrUX 现场数据),必须在 data_source 注明『实验室合成值·非现场数据·仅供参考』,严禁表述为『实测优秀 / CWV 健康』并当作核心优势——单次合成 LCP 不代表真实用户体验**;"
        ...
        "⑥ 视频专项(**仅当确为视频站时启用,否则整节跳过/标 N/A**):视频可被抓(不被 robots 封/不藏登录后)、VideoObject、视频 sitemap、缩略图唯一——综合 ④⑤,视频不可索引或无 VideoObject 才是头号缺陷。\n"
...
async def _network_synthesize(ctx: Any, state: dict[str, Any]) -> dict[str, Any]:
    ...
    system = (
        ...
        "【业务类型判定 · 最高优先级 · 防臆测,违者结论作废】被测业务/页面类型必须**仅从输入材料(业务说明)与实测数据(实际页面内容/操作)判定**,"
        "**严禁预设或脑补任何品类**(尤其不得无依据地把站点当成『视频站』而臆造起播/卡顿/ABR/分片续播等视频弱网问题)。"
        "**仅当输入材料明确是视频业务、或实测数据里含『# 视频播放弱网实测证据』时**,才做视频相关分析;否则视频维度一律不适用,不得出现。"
        "若『输入业务描述』与『实测对象(URL)』不一致(如说明写后台登录页、URL 却是静态占位页),**必须在 verdict_summary / risks 明确指出该矛盾**,并据实测对象的真实形态下结论,不得自行脑补业务画像。\n\n"
