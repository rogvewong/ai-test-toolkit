...
## 对**每一个**接口,逐维度真发请求(适用即测,不许用"等/类似"含糊带过)
...
### 3. 类型校验(逐个字段,传错类型)
- 把每个字段传成错误类型(数字字段传字符串、字符串字段传数字/对象、布尔传字符串、数组字段传标量…) → 断言是否被正确拒绝(4xx + 错误码),还是被错误地强转/接受。

### 4. 枚举校验(每个合法值 + 非法值)
- 枚举字段:**每个合法枚举值各发一次**确认都能被接受;再传非枚举值 / 大小写变体(如 `PENDING` vs `pending`)/ 中英文混 → 断言非法值被拒、错误码明确。

### 5. 响应字段契约(对照文档逐字段核真实响应)
- 拿正常流的真实响应,逐字段核:字段名 / 类型 / 层级是否与文档一致;`format`(uuid / date-time / email / uri)是否合规;枚举字段返回值是否在声明范围内;数值范围(如 `amount_cents >= 0`)。
- 文档声明的字段是否真返回(漏字段);是否返回了**未声明字段**(尤其是疑似敏感字段,移交 4_3 复核但此处先记契约不一致)。
- 不同场景下 nullable / optional 字段的存在性是否稳定(如空列表 vs 非空列表的 `data` 结构)。

### 6. 状态码语义(逐接口核真实返回码)
- 成功是否用对码(查询 200、创建 201、无内容 204);客户端错误是否 4xx 而非 200+错误体或 500;鉴权失败 401/403;资源不存在 404;方法不允许 405;Content-Type 不支持 415。核对**真实返回码**是否语义正确。
...
> 提醒:一个接口的一个维度往往要发**多个**请求(必填有几个字段就发几次、枚举有几个值就发几次)。逐个真发,记录每条真实响应。
